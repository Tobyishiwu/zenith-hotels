  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        {loading ? (
          <p className="text-center font-body text-primary/50">Loading gallery...</p>
        ) : (
          <>
      ...
      <AnimatePresence>
        {lightboxIndex !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-primary/95 z-[100] flex items-center justify-center px-6">
            <button onClick={closeLightbox} className="absolute top-6 right-6 text-white" aria-label="Close"><X size={32} /></button>
            <button onClick={showPrev} className="absolute left-4 md:left-10 text-white" aria-label="Previous image"><ChevronLeft size={36} /></button>
            <motion.img
              key={filteredItems[lightboxIndex]._id}
            ...
            />
            <button onClick={showNext} className="absolute right-4 md:right-10 text-white" aria-label="Next image"><ChevronRight size={36} /></button>
